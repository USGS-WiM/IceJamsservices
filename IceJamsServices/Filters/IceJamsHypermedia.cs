...
namespace IceJamsServices.Filters
{
    public class IceJamsHypermedia : HypermediaBase
    {
        ...
        protected override List<Link> GetReflectedHypermedia(IHypermedia entity)
        {
            ...
            switch (entity.GetType().Name)
            {
                case "IceJams":
                    results = new List<Link>();
                    results.Add(new Link(BaseURI, "icejams example", this.URLQuery + "/", WiM.Resources.refType.POST));
