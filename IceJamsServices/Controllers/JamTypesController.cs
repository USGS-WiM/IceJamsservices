...
{
    [Route("[controller]")]
    public class JamTypesController : ControllerBase
    {
        public JamTypesController(IIceJamsAgent agent ) : base(agent)
        {
        }
        #region METHODS
        [HttpGet]
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                ...
                return Ok(await agent.GetJamType(id));
            }
            catch (Exception ex)
            {
                return await HandleExceptionAsync(ex);
        ...
        [Authorize(Policy = "AdminOnly")]
        [Route("Batch")]
        public async Task<IActionResult> Batch([FromBody]List<JamType> entities)
        {
            try
            {
